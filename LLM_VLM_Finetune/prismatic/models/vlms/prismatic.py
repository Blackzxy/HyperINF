...
class PrismaticLLM(VLM):
    ...
    def freeze_backbones(self, stage: str) -> None:
        ...
        if stage == "llama-dataset-pruning":
            ## second we only choose the LLM's parameters as the theta for 
            ## the data pruning step
            self.llm_backbone.requires_grad_(False)

            for k,v in self.llm_backbone.named_parameters():
                ### only set the embedding layer, first 5 layers and the last 5 layers to be trainable
                if "layers.31" in k:
            ...
            raise ValueError(f"Stage `{stage}` is not supported for LLaMa2! Try < align | finetune >")
...
class PrismaticVLM(VLM):
    ...
    def val_prompt_fn(self, dataset_family: str = "text-vqa") -> Callable[[str], str]:
    ...
    def get_captioning_prompt_fn(self) -> Callable[[str], str]:
        ...
        prompt_builder_fn = self.get_prompt_builder
